...
//I2C Settings
TwoWire I2CdistanceSensorPin = TwoWire(0);
...
VL53L0X::VL53L0X(void)
  ...
  , did_timeout(false)
{
  I2CdistanceSensorPin.begin(27,32,100000);
}
...
{
  I2CdistanceSensorPin.beginTransmission(address);
  I2CdistanceSensorPin.write(reg);
  I2CdistanceSensorPin.write(value);
  last_status = I2CdistanceSensorPin.endTransmission();
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  I2CdistanceSensorPin.beginTransmission(address);
  I2CdistanceSensorPin.write(reg);
  I2CdistanceSensorPin.write((value >> 8) & 0xFF); // value high byte
  I2CdistanceSensorPin.write( value       & 0xFF); // value low byte
  last_status = I2CdistanceSensorPin.endTransmission();
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  I2CdistanceSensorPin.beginTransmission(address);
  I2CdistanceSensorPin.write(reg);
  I2CdistanceSensorPin.write((value >> 24) & 0xFF); // value highest byte
  I2CdistanceSensorPin.write((value >> 16) & 0xFF);
  I2CdistanceSensorPin.write((value >>  8) & 0xFF);
  I2CdistanceSensorPin.write( value        & 0xFF); // value lowest byte
  last_status = I2CdistanceSensorPin.endTransmission();
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  I2CdistanceSensorPin.requestFrom(address, (uint8_t)1);
  value = I2CdistanceSensorPin.read();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  I2CdistanceSensorPin.requestFrom(address, (uint8_t)2);
  value  = (uint16_t)I2CdistanceSensorPin.read() << 8; // value high byte
  value |=           I2CdistanceSensorPin.read();      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  I2CdistanceSensorPin.requestFrom(address, (uint8_t)4);
  value  = (uint32_t)I2CdistanceSensorPin.read() << 24; // value highest byte
  value |= (uint32_t)I2CdistanceSensorPin.read() << 16;
  value |= (uint16_t)I2CdistanceSensorPin.read() <<  8;
  value |=           I2CdistanceSensorPin.read();       // value lowest byte
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
  ...
  while (count-- > 0)
  {
    I2CdistanceSensorPin.write(*(src++));
  }
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  I2CdistanceSensorPin.requestFrom(address, count);

  while (count-- > 0)
  {
    *(dst++) = I2CdistanceSensorPin.read();
  }
}
